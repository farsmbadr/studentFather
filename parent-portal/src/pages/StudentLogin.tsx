  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: err } = await supabase
        .from('students')
        .select('id, name, code, grade, group_name, phone, status')
        .eq('code', code.trim())
        .eq('phone', phone.trim())
        .maybeSingle();
      if (err) throw err;
      ...
      sessionStorage.setItem('portal_student', JSON.stringify(data));
      sessionStorage.setItem('portal_type', 'student');
      ...
      <div className="w-full max-w-md">
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 p-8">
          ...
          <div className="flex justify-center gap-4 mt-4">
            <Link to="/parent" className="text-white/30 hover:text-white/60 text-xs transition-colors">دخول ولي الأمر</Link>
          </div>
        </div>
