...
export default function ParentDashboard() {
  ...
  const stored = sessionStorage.getItem('portal_parent');
  const studentInfo = stored ? JSON.parse(stored) : null;

  useEffect(() => {
    if (!studentInfo) { navigate('/parent', { replace: true }); return; }
    loadDashboard();
    loadMessages();
  }, []);
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !studentInfo) return;
    setSendingMsg(true);
    try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900" dir="rtl">
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <StatCard icon={<DollarSign size={16} className="text-white" />} label="المتبقي" value={`${(student.total_outstanding || 0).toLocaleString()} ج`} color={student.total_outstanding > 0 ? 'from-red-500 to-rose-600' : 'from-green-500 to-emerald-600'} />
        ...
        <Section id="payments" title={`المصروفات — ${student.total_outstanding > 0 ? 'إجمالي المتأخر ' + student.total_outstanding.toLocaleString() + ' ج' : 'مدفوع بالكامل'}`} icon={<DollarSign size={14} className="text-white" />} color="from-emerald-500 to-teal-600">
        ...
        <div className="bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
          <button onClick={() => setActiveSection(activeSection === 'messages' ? null : 'messages')}
            className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-pink-600 flex items-center justify-center"><MessageSquare size={14} className="text-white" /></div>
              <span className="text-white font-semibold text-sm">رسائلي إلى المركز</span>
            </div>
            <div className={`text-white/30 transition-transform ${activeSection === 'messages' ? 'rotate-180' : ''}`}>▼</div>
          </button>
              ...
              <div className="flex items-end gap-2 mb-4">
                <div className="flex-1">
                  <textarea value={newMessage} onChange={e => setNewMessage(e.target.value)}
                    placeholder="اكتب رسالتك إلى الإدارة..." rows={2}
                  ...
                  {messages.map((m: any) => (
                    <div key={m.id} className={`rounded-xl px-3.5 py-2.5 ${m.reply ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-white/5'}`}>
                      <div className="flex items-start justify-between">
                        ...
                        <span className="text-white/20 text-[10px] shrink-0 mr-2">{new Date(m.created_at).toLocaleDateString('ar-EG')}</span>
                      </div>
                      {m.reply && (
