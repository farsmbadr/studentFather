...
export default function ParentLogin({ onLogin }: ParentLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const r = await fetch('/api/auth/parent-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code, parentPhone }),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-4" dir="rtl">
      <div className="w-full max-w-md">
        ...
        <p className="text-center text-white/20 text-xs mt-6">v1.0.0 &copy; 2026 — مركز بدر</p>
      </div>
    </div>
  );
}
