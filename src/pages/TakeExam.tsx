...
export default function TakeExam() {
  ...
  useEffect(() => {
    ...
    const match = path.match(/\/take-exam\/(.+)/);
    if (!match) { setError('رابط الاختبار غير صالح'); setLoading(false); return; }
    const eid = match[1];
    setExamId(eid);

    (async () => {
      const [examRes, subRes, qRes] = await Promise.all([
        supabase.from('exams').select('*').eq('id', eid).maybeSingle(),
      ...
      const subject = (subRes.data || []).find((s: any) => s.name === exam.subject);
      const subjectId = subject?.id;

      let qs: any[] = [];

      if (exam.selection_mode === 'auto') {
        const ac = exam.auto_config || {};
        ...
        const eqRes = await supabase.from('exam_questions').select('*').eq('exam_id', eid);
        const qIds = (eqRes.data || []).map((eq: any) => eq.question_id);
      ...
      setQuestions(qs);
      setTotal(qs.length);
      if (exam.duration > 0 && !(exam.closing_date && new Date(exam.closing_date) < new Date())) setTimeLeft(exam.duration * 60);
      setLoading(false);
    })();
  }, []);
  ...
  const handleIdentify = async () => {
    ...
    try {
      const { data } = await supabase.from('students').select('*').eq('status', 'active').eq('code', studentCode.trim()).eq('phone', studentPhone.trim()).maybeSingle();
      if (data) {
        setStudentId(data.id);
        setStudentName(data.name);
        const currentExamTitle = exam?.title || '';
        const { data: existing } = await supabase.from('exam_results').select('*').eq('student_id', data.id).eq('exam_title', currentExamTitle).maybeSingle();
        if (existing) {
  ...
  if (alreadyTaken) return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-xl shadow-indigo-100 p-8 max-w-md w-full text-center">
        <div className="w-16 h-16 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-4"><AlertTriangle size={32} className="text-amber-500" /></div>
        <h2 className="text-xl font-bold text-gray-800 mb-2">لقد أديت هذا الاختبار من قبل</h2>
        <p className="text-sm text-gray-500">لا يمكنك دخول الاختبار أكثر من مرة</p>
      </div>
    </div>
  );
          ...
          <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400">{studentName}</p>
              </div>
            </div>
                  ...
                  return (
                    <div className="px-6 pb-6 space-y-2">
                      {q.options.map((opt: string, oi: number) => {
                        ...
                        return (
                          <div key={oi} className={`flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all ${
                            isCorrect ? 'border-emerald-400 bg-emerald-50' :
                            isWrong ? 'border-red-300 bg-red-50' : 'border-gray-100'
                          }`}>
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                              isCorrect ? 'border-emerald-500 bg-emerald-500' :
                              isWrong ? 'border-red-500 bg-red-500' : 'border-gray-300'
                            }`}>
                              {(isCorrect || isWrong) && <div className="w-2 h-2 rounded-full bg-white" />}
                            </div>
                            <span className={`text-sm ${
                              isCorrect ? 'text-emerald-700 font-semibold' :
                              isWrong ? 'text-red-700 font-semibold' : 'text-gray-700'
                            }`}>{opt}</span>
                    ...
                    <div className="px-6 pb-6">
                      <div className="grid grid-cols-2 gap-3">
                        {['صح', 'خطأ'].map((val: string) => {
                          ...
                          const isWrong = isStudentChoice && !isCorrect;
                          return (
                            <div key={val} className={`flex items-center justify-center gap-2 p-4 rounded-xl border-2 transition-all ${
                              isCorrect ? 'border-emerald-400 bg-emerald-50' :
                              isWrong ? 'border-red-300 bg-red-50' : 'border-gray-100'
                            }`}>
                              ...
                              <span className={`text-sm font-semibold ${
                                ...
                                isWrong ? 'text-red-700' : 'text-gray-700'
                              }`}>{val}</span>
